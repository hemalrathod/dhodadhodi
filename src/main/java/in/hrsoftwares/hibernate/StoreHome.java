 ...
 * @see in.hrsoftwares.hibernate.Store
...
public class StoreHome {
	...
	public Store findById(java.lang.Byte id) {
		...
		try {
			Store instance = (Store) sessionFactory.getCurrentSession().get("in.hrsoftwares.hibernate.Store", id);
			if (instance == null) {
	...
	public List<Store> findByExample(Store instance) {
		...
		try {
			List<Store> results = (List<Store>) sessionFactory.getCurrentSession()
					.createCriteria("in.hrsoftwares.hibernate.Store").add(create(instance)).list();
