 ...
 * @see in.hrsoftwares.hibernate.Inventory
...
public class InventoryHome {
	...
	public Inventory findById(java.lang.Integer id) {
		...
		try {
			Inventory instance = (Inventory) sessionFactory.getCurrentSession()
					.get("in.hrsoftwares.hibernate.Inventory", id);
			if (instance == null) {
	...
	public List<Inventory> findByExample(Inventory instance) {
		...
		try {
			List<Inventory> results = (List<Inventory>) sessionFactory.getCurrentSession()
					.createCriteria("in.hrsoftwares.hibernate.Inventory").add(create(instance)).list();
