 ...
 * @see in.hrsoftwares.hibernate.City
...
public class CityHome {
	...
	public City findById(java.lang.Short id) {
		...
		try {
			City instance = (City) sessionFactory.getCurrentSession().get("in.hrsoftwares.hibernate.City", id);
			if (instance == null) {
	...
	public List<City> findByExample(City instance) {
		...
		try {
			List<City> results = (List<City>) sessionFactory.getCurrentSession()
					.createCriteria("in.hrsoftwares.hibernate.City").add(create(instance)).list();
