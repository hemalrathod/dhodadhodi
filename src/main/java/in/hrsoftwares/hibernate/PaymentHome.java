 ...
 * @see in.hrsoftwares.hibernate.Payment
...
public class PaymentHome {
	...
	public Payment findById(java.lang.Short id) {
		...
		try {
			Payment instance = (Payment) sessionFactory.getCurrentSession().get("in.hrsoftwares.hibernate.Payment", id);
			if (instance == null) {
	...
	public List<Payment> findByExample(Payment instance) {
		...
		try {
			List<Payment> results = (List<Payment>) sessionFactory.getCurrentSession()
					.createCriteria("in.hrsoftwares.hibernate.Payment").add(create(instance)).list();
