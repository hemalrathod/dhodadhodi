 ...
 * @see in.hrsoftwares.hibernate.Address
...
public class AddressHome {
	...
	public Address findById(java.lang.Short id) {
		...
		try {
			Address instance = (Address) sessionFactory.getCurrentSession().get("in.hrsoftwares.hibernate.Address", id);
			if (instance == null) {
	...
	public List<Address> findByExample(Address instance) {
		...
		try {
			List<Address> results = (List<Address>) sessionFactory.getCurrentSession()
					.createCriteria("in.hrsoftwares.hibernate.Address").add(create(instance)).list();
