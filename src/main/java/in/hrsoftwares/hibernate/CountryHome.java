 ...
 * @see in.hrsoftwares.hibernate.Country
...
public class CountryHome {
	...
	public Country findById(java.lang.Short id) {
		...
		try {
			Country instance = (Country) sessionFactory.getCurrentSession().get("in.hrsoftwares.hibernate.Country", id);
			if (instance == null) {
	...
	public List<Country> findByExample(Country instance) {
		...
		try {
			List<Country> results = (List<Country>) sessionFactory.getCurrentSession()
					.createCriteria("in.hrsoftwares.hibernate.Country").add(create(instance)).list();
