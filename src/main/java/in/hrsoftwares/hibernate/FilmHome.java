 ...
 * @see in.hrsoftwares.hibernate.Film
...
public class FilmHome {
	...
	public Film findById(java.lang.Short id) {
		...
		try {
			Film instance = (Film) sessionFactory.getCurrentSession().get("in.hrsoftwares.hibernate.Film", id);
			if (instance == null) {
	...
	public List<Film> findByExample(Film instance) {
		...
		try {
			List<Film> results = (List<Film>) sessionFactory.getCurrentSession()
					.createCriteria("in.hrsoftwares.hibernate.Film").add(create(instance)).list();
